#version 120
// ============================================================================
// Faithful GLSL port of vp_worldtex_env_fog_t2 (ARB) — walls/static geometry
// with lightmap + envmap + fog. THE VP the engine binds with fp_worldtex_env_fog.
// Pairs with fp_worldtex_env_reflective / fp_worldtex_lm_env.
// See deploy/shaders_override/vp_worldtex_env_fog_t2.txt for the ARB source.
//
// KEY DIFFERENCE from vp_static_env_fog.vs.glsl: lightmap UV comes from
// gl_MultiTexCoord1 (ARB: MOV result.texcoord[1], vertex.texcoord[1]), NOT
// gl_MultiTexCoord2. Getting this wrong maps the lightmap at the wrong UV →
// wrong lighting on walls. tc3 = gl_MultiTexCoord3 passthrough.
// ============================================================================
...
void main()
{
    ...
    // --- Cube reflect coord (world space) → tc2 ---
    float NdE = dot(eyeN, eye.xyz);
    vec3 R = eye.xyz - NdE * eyeN * 2.0;
    R = normalize(R);
    vec3 wR;
    wR.x = dot(R, uVI0.xyz);
    wR.y = dot(R, uVI1.xyz);
    wR.z = dot(R, uVI2.xyz);
    gl_TexCoord[2] = vec4(wR, 1.0);

    // --- Stock texcoords for walls variant ---
    gl_TexCoord[0] = gl_MultiTexCoord0;   // diffuse UV
    gl_TexCoord[1] = gl_MultiTexCoord1;   // lightmap UV (gl_MultiTexCoord1, NOT 2!)
    gl_TexCoord[3] = gl_MultiTexCoord3;   // passthrough
