#version 120
// Faithful GLSL port of fp_model_bump_env_spec ARB + shadow_receive_self.inc.
// Pairs with vp_skinned_env_lit.vs.glsl (reuses same VS as env_reflective — identical
// bone/lights/fog/cube-reflect structure). Droids/characters with normalmap + envmap + spec.
// TMU: tex0=0, texEnv=1(cube), texNrm=8, texRgh=9, texMtl=10, texShadow=5(SELF)
...
void main()
{
    ...
    float mt = (pbr.x < 0.0) ? 1.0 : pbr.x;   // model bump_env defaults metal=1
    ...
    // Stock-K2 base: r = v*d (char has no lightmap)
    vec3 r = v * d.rgb;
    ...
    // Sun diffuse — gated by fl.x
    float sd = max(dot(N, tnD.xyz), 0.0) * tnE.x * fl.x * shadowed;
    r += (d.rgb * tnE.yzw) * sd;
