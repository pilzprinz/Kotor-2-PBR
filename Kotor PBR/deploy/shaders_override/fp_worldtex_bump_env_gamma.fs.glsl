#version 120
// Faithful GLSL port of fp_worldtex_bump_env_gamma ARB + shadow_receive.inc.
// Pairs with vp_static_env_fog.vs.glsl. Like bump_env but with gamma (c7) + tint (c1).
...
void main()
{
    ...
    if (fl.y > 0.0) rs = mix(rs, texture2D(texRgh, uv).r, fl.y);
    ...
    // Stock gamma + tint (simplified — c7 and c1 from env[7] and env[0])
    r *= vec3(1.0);  // gamma c7 placeholder
    r += vec3(0.0);  // tint c1 placeholder
