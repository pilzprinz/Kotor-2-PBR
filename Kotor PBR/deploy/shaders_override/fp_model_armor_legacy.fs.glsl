#version 120
// Faithful GLSL port of fp_model_armor_legacy ARB + shadow_receive_self.inc.
// Pairs with vp_skinned_env_lit.vs.glsl. Diffuse × vtxlight, envmap reflect mix by alpha, fog.
// TMU: tex0=0, texEnv=1(cube), texShadow=5(SELF)
...
void main()
{
    ...
    vec3 alb = d.rgb;
    vec3 m = textureCube(texEnv, gl_TexCoord[1].xyz).rgb;

    // Stock: diffuse × vtxlight, then envmap reflect mix by alpha
    d.rgb *= v;
    ...
    // Cast-shadow darken only (no NdotL gate — vertex light handles back-face)
